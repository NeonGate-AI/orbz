...
| --- | --- | --- |
| `voiceEngine` | `OrbzVoiceEnginePort \| undefined` | `WebSpeechAdapter` |
...
Set a custom engine or flow before appending a programmatically created
element so the mandatory first utterance uses that configuration.
Assigning `undefined` restores the corresponding built-in default.
...
An invalid preset normalizes to `neongate`. The public API is named `preset`;
...
Orbz automatically calls `startTalking()` after its first connection. The
default flow stores the captured name only in the current element instance.
See [Voice and talk runtime](/guides/voice-assistant) for custom flows and
engines.
...
See [Package exports](/api/exports) for the constants and TypeScript types.
